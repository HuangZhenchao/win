...
WCHAR* GetFileParentPath(WCHAR *lpszFilePath) {
	WCHAR *lpszFileParentPath = new WCHAR[MAX_PATH];
	ZeroMemory(lpszFileParentPath, MAX_PATH);
	vector<wstring> vSplitStrings;
	vSplitStrings = split(lpszFilePath, L"\\");
	if (vSplitStrings.size() > 1) {
		wcscat_s(lpszFileParentPath, MAX_PATH, vSplitStrings[0].c_str());
		for (int i = 1; i < vSplitStrings.size()-1; i++) {
			wcscat_s(lpszFileParentPath, MAX_PATH, L"\\");
			wcscat_s(lpszFileParentPath, MAX_PATH, vSplitStrings[i].c_str());			
		}
	}
	else {
		wcscat_s(lpszFileParentPath, MAX_PATH, vSplitStrings[0].c_str());
	}
	return lpszFileParentPath;
}
...
WCHAR* GetFirstDriverCharW(WCHAR *lpszFilePath) {
	WCHAR *lpszFirstChar = new WCHAR[MAX_PATH];
	ZeroMemory(lpszFirstChar, MAX_PATH);
	CopyMemory(lpszFirstChar, lpszFilePath, sizeof(WCHAR));
	return lpszFirstChar;
}

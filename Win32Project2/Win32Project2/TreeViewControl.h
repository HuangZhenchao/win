#pragma once
#include <map>
#include "TreeViewControl\TreeViewData.h"
class TREEVIEWSTYLE {
public:
	HINSTANCE hInstance;
	HWND hWndParent;
...
TREEVIEWCONTROL::TREEVIEWCONTROL(DWORD dataSource, wstring rootPath, wstring defaultPath)
{
	TreeViewData =new TREEVIEWDATA(dataSource, rootPath, defaultPath);
	m_id = m_map.size() + 1;
...
HWND TREEVIEWCONTROL::CreateTreeViewControl(TREEVIEWSTYLE *tvStyle)
{
	...
	HWND hWndControl= CreateWindowEx(NULL, L"MyTreeViewControl", NULL, WS_CHILD | WS_VISIBLE,
	...
	for (DWORD i = 0; i < TreeViewData->vTreeNode.size(); i++) {
		...
		vHWNDItem.push_back(CreateWindowEx(NULL, L"MyTreeViewItem", NULL, WS_CHILD | WS_VISIBLE,
			0, 0, 0, 0, hWndControl, (HMENU)dItemID, tvStyle->hInstance, NULL));

		SIZE sizeTest;
		GetTextExtentPoint32(hdc, TreeViewData->vTreeNode[i]->data.NodeName.c_str(), wcslen(TreeViewData->vTreeNode[i]->data.NodeName.c_str()), &sizeTest);
		DWORD ItemWidth = TVStyle->dIndent*(TreeViewData->vTreeNode[i]->data.dNodeDepth) + sizeTest.cx;//sizeTest.cx
...
LRESULT CALLBACK  TREEVIEWCONTROL::Proc_WndTreeViewItem(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		//MessageBox(NULL, tempStr1, TEXT("错误码"), MB_OK | MB_ICONINFORMATION);
		TextOut(hdc, pThis->TVStyle->dIndent*(pThis->TreeViewData->vTreeNode[v_id]->data.dNodeDepth), 0,
			pThis->TreeViewData->vTreeNode[v_id]->data.NodeName.c_str(),
			wcslen(pThis->TreeViewData->vTreeNode[v_id]->data.NodeName.c_str()));
